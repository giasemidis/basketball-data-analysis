...
df['Win'] = np.where(df['Home Score'] > df['Away Score'], 'W', 'L')
df['Win'] = np.where(df['Home Score'] < df['Away Score'], 'W', 'L')
...
fig = px.box(df, x="Season", y="Home Score", color="Result", notched=True)
plot(fig, 'plot1')

fig = px.box(df, x="Season", y="Away Score", color="Result", notched=True)
plot(fig, 'plot2')

# %%
fig = px.histogram(df_flat, x='Score', y='Score', color='Loc',
