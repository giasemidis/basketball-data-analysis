...
def make_standings(results, nround):
    ...
    home = results.groupby(['Home Team ID'])['Home Points', 'Home Score Regular Period', 'Away Score Regular Period'].sum()
    away = results.groupby(['Away Team ID'])['Away Points', 'Away Score Regular Period', 'Home Score Regular Period'].sum()
    ...
    standing.sort_values(by=['Points','Score Diff', 'Score+'], inplace=True, ascending=False)
    ...
    for p in np.unique(standing['Points'].values):
        if np.sum(standing['Points'].values == p) > 1:
            ...
            home = minichamp.groupby(['Home Team ID'])['Home Points', 'Home Score Regular Period', 'Away Score Regular Period'].sum()
            away = minichamp.groupby(['Away Team ID'])['Away Points', 'Away Score Regular Period', 'Home Score Regular Period'].sum()
            groupby = home.merge(away, how='outer', left_index=True, right_index=True)
            ...
            points = groupby['Home Points'].values + groupby['Away Points'].values
    ...
    standing.sort_values(by=['Points', 'Secondary Points', 'Secondary Score Diff',
                             'Score Diff'], inplace=True, ascending=False)
    standing.reset_index(drop=True, inplace=True)
