...
def kfold_crosseval(X_train, y_train, df_train, nsplits, groups=None,
    ...
    for train_index, test_index in folditer:
        # loop over folds
        i += 1
        X_train_folds, X_test_fold = (X_train[train_index, :],
                                      X_train[test_index, :])
        ...
        if level == 'team':
            ...
            for gid in np.unique(df_test_fold['Game ID']):
                ...
                if teams.shape[0] == 2:
                    game_pred = (1 if teams.iloc[0]['Prob'] >
                                 teams.iloc[1]['Prob'] else 0)
                    game_resu = (1 if teams.iloc[0]['Label'] >
                                 teams.iloc[1]['Label'] else 0)
